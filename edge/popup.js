// popup.js - Edge-compatible version
document.addEventListener('DOMContentLoaded', function() {
    // Detect browser type
    const isEdge = navigator.userAgent.includes("Edg");
    
    // Log browser detection for debugging
    console.log('Browser detection:', { isEdge });
  ...
  // Save reminder to storage with error handling for Edge
  function saveReminder(reminder) {
    try {
      ...
      remindersList.innerHTML = `<div class="error-message">Error saving reminder: ${error.message}</div>`;
    }
  }
  
  // Load reminders from storage and display with error handling for Edge
  function loadReminders() {
    ...
    try {
      ...
      remindersList.innerHTML = `<div class="error-message">Error loading reminders: ${error.message}</div>`;
    }
  }
  ...
  function editReminder(id) {
    try {
      ...
      alert('Failed to edit reminder: ' + error.message);
    }
  }
  ...
  function completeReminder(id) {
    try {
      ...
      alert('Failed to complete reminder: ' + error.message);
    }
  }
      ...
      try {
        chrome.runtime.sendMessage({
          ...
          if (!response || !response.success) {
            console.error('Failed to schedule reminder:', response?.error || 'Unknown error');
          }
        });
      } catch (error) {
